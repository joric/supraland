  ...
  L.Hash.formatHash = function(map) {
    ...
    for(var key in options) {
      //if (options.hasOwnProperty(key)) {
        ...
        //console.log('map haslayer', key, map.hasLayer(options[key]));
  ...
  L.Hash.prototype = {
    ...
    onMapMove: function() {
      ...
      if (this.lastHash != hash) {
        ...
        if (parsed && parsed.layers && parsed.layers[0]) {

          //console.log('parsed', parsed.layers);

          let root = parsed.layers[0];
          settings[root] = hash;
          settings['root'] = root;

          //console.log('writing root', root, 'hash', hash);
    ...
    update: function() {
      ...
      if (parsed) {
        ...
        try {
          ...
          //console.log('layers', layers, 'map', this.map);
          ...
          // handle situation with non-existing layers
